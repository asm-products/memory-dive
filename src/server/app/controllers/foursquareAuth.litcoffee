...
# FoursquareAuth controller

This controller is responsible for authorizing the user with Foursquare service.

## Constants

The app's provider configuration is read from the process environment.

    CLIENT_ID      = process.env.MEMORY_DIVE_FOURSQUARE_CLIENT_ID
    CLIENT_SECRET  = process.env.MEMORY_DIVE_FOURSQUARE_CLIENT_SECRET
...
We use a specialized strategy name to avoid any conflicts with *authentication* strategy we use with through a different mechanism.
    ...
    PASSPORT_STRATEGY_NAME = 'memory-dive-foursquare-auth'

## Initialization

    _           = require 'lodash'
    debug       = (require 'debug') 'memdive::controllers::foursquareAuth'
    api1UrlCreator  = require '../common/api1UrlCreator'
    constants   = require '../common/constants'
    passport    = require 'passport'

FoursquareAuthController inherits ProviderAuthBaseController.

    FoursquareAuthController = (init) ->
        ProviderAuthBaseController.call this, init
        configPassport()
        return

    ProviderAuthBaseController = require('./providerAuthBase')
    require('util').inherits FoursquareAuthController, ProviderAuthBaseController
    module.exports = FoursquareAuthController

## Private functions

    getCallbackUrl = ->
        return process.env.MEMORY_DIVE_BASE_URL + api1UrlCreator.getDataProviderAuthCallbackPath(constants.providerId.FOURSQUARE)
...
Both of these query params are obligatory.
    ...
    FoursquareAuthController::add = (c) ->
...
`callback` is invoked by the provider during the OAuth authroization process.
    ...
    FoursquareAuthController::callback = (c) ->
        ...
        if c.req.query and c.req.query.error
            return c.redirectError constants.error.FOURSQUARE_AUTH_ERROR, c.req.query.error, failureUrl
...
Our specialized authorization handler will save the new (or updated) provider's data and invoke data collection.
        ...
        handleAuthorization = (err, user, info) ->
            ...
            providerData =
                ...
                providerId:     constants.providerId.FOURSQUARE
                providerUserId: info.profile.id
                displayName:    info.profile.name?.givenName + ' ' + info.profile.name?.familyName
                # We get the photo in all its original size
                pictureUrl:     info.profile._json?.response?.user?.photo?.prefix + 'original' + info.profile._json?.response?.user?.photo?.suffix
                providerData:
