...
# DropboxBackupAuth controller

This controller is responsible for authorizing the user with Dropbox service for the purposes of user backups.

## Constants

The app's provider configuration is read from the process environment.

    APP_KEY     = process.env.MEMORY_DIVE_DROPBOX_BACKUP_APP_KEY
    APP_SECRET  = process.env.MEMORY_DIVE_DROPBOX_BACKUP_APP_SECRET
...
We use a specialized strategy name to avoid any conflicts with *authentication* strategy we use with through a different mechanism.
    ...
    PASSPORT_STRATEGY_NAME = 'memory-dive-dropbox-backup-auth'

## Initialization

    _           = require 'lodash'
    debug       = (require 'debug') 'memdive::controllers::DropboxBackupAuth'
    api1UrlCreator = require '../common/api1UrlCreator'
    constants   = require '../common/constants'
    passport    = require 'passport'

DropboxBackupAuthController inherits ProviderAuthBaseController.

    DropboxBackupAuthController = (init) ->
    ...
    require('util').inherits DropboxBackupAuthController, ProviderAuthBaseController
    module.exports = DropboxBackupAuthController

## Private functions

    getCallbackUrl = ->
        return process.env.MEMORY_DIVE_BASE_URL + api1UrlCreator.getBackupProviderAuthCallbackPath(constants.providerId.DROPBOX)
...
Both of these query params are obligatory.
    ...
    DropboxBackupAuthController::add = (c) ->
...
`callback` is invoked by the backup destination during the OAuth authroization process.

    DropboxBackupAuthController::callback = (c) ->
...
Our specialized authorization handler will save the new (or updated) backup destination's data and invoke immediate backup.
        ...
        handleAuthorization = (err, user, info) ->
            ...
            c.compound.models.UserBackupProvider.putUserProvider providerData, (error, provider) ->
                ...
                c.compound.common.scheduler.backupUserDataToProvider provider, (err) ->
                    debug 'Backup of user data failed', provider, err if err
